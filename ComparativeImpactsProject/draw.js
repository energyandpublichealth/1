...
var nucPerc = .43;  //NYC portfolio
var natgasPerc = .57;  //NYC portfolio
var coalPerc = 0;
var solarPerc = 0;
var windPerc = 0;
...
function calcKWH(bill){  //break down electric bill into kWh
  ...
  longkWh = bill / .2321  //23.21 cents is avg. cost per kWh in NYC
...
function calcCO2(natGas, nuclear, coal, solar, wind, manip){  //calculate CO2 emissions total and by energy source
        ...
        natGasCO2 = natGas * 460;  //460 g CO2 eq/kWh on average from DoE 2010
        inputVals[4].co2N = natGasCO2;  //add value to data set

        nuclearCO2 = nuclear * 66;  //66 g CO2 eq/kWh from DoE 2010
        inputVals[2].co2N = nuclearCO2;

        coalCO2 = coal * 915; //avg. 915g CO2 eq from DoE 2010
        inputVals[3].co2N = coalCO2;

        solarCO2 = solar * 37.5;  //37.5 avg from DoE 2010
        inputVals[0].co2N = solarCO2;

        windCO2 = wind * 15.5;  //15.5 avg from DoE 2010
...
function calcEnvirCosts(CO2, manip){
    envirCost = CO2 * 0.00004; //($0.00004/gram of CO2)
...
function calcLandUse(natGas, nuclear, coal, solar, wind, manip){
  ...
  nGLand = natGas * 0.000000005015;

  nLand = nuclear * 0.00000000013;

  cLand = coal * 0.000000004415;

  sLand = solar * 0.00000001501;

  wLand = wind * 0.00000012692;
